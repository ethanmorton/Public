...
!---------------------------------------------------------------------------------------------------------------------------
        use constants
        IMPLICIT NONE
        real*8 pos_e(2),pos_j(2),vel_e(2),vel_j(2),me,mj,ms,pe,pj,dt
        integer nt

    !
    !   Open up the output file
    !
        open(u,file='output_3Body.dat')

        call initialize(pos_e,pos_j,vel_e,vel_j,ms,me,mj,pe,pj,dt,nt)
        call calculate(pos_e,pos_j,vel_e,vel_j,pe,pj,dt,nt)

    end program threebody

!===========================================================================================================================
   subroutine initialize(pos_e,pos_j,vel_e,vel_j,ms,me,mj,pe,pj,dt,nt)
!===========================================================================================================================
        use constants
        IMPLICIT NONE
        real*8 pos_e(*),pos_j(*),vel_e(*),vel_j(*),me,mj,ms,e_e,e_j,pj,pe,dt,r_cm,v_cm
        integer nt

    !
    !   Set up initial conditions (in AU units): 
    !
        open(1,file='input_3Body.dat')
    ...
    !
        ...
        pos_e(2) = 0.d0
        vel_e(1) = 0.d0
        vel_e(2) = 2.d0*pi*sqrt((1+e_e)/abs(pos_e(1))/(1-e_e)*(1))
        
        pos_j(1) = pos_j(1)*(1-e_j)
        pos_j(2) = 0.d0
        vel_j(1) = 0.d0    
        vel_j(2) = 2.d0*pi*sqrt((1+e_j)/abs(pos_j(1))/(1-e_j)*(1))

   end subroutine initialize

!===========================================================================================================================
   subroutine calculate(pos_e,pos_j,vel_e,vel_j,pe,pj,dt,nt) 
!===========================================================================================================================
! use the Euler-Cromer method; update the velocities first
!---------------------------------------------------------------------------------------------------------------------------
        use constants
        IMPLICIT NONE
        real*8 me,mj,ms,pj,pe,pos_e(*),pos_j(*),vel_e(*),vel_j(*),t,dt,r_es,r_js,r_ej
        integer nt,i
    ...
    !
        write(u,'(10f14.7)')  t,pos_e(1),pos_e(2),pos_j(1),pos_j(2)

        do  i=1,nt
            r_es     = sqrt(pos_e(1)**2 + pos_e(2)**2)
            r_js     = sqrt(pos_j(1)**2 + pos_j(2)**2)
            r_ej     = sqrt((pos_e(1)-pos_j(1))**2 + (pos_e(2)-pos_j(2))**2)
    !
    !       Update velocities first
    !
            vel_e(1) = vel_e(1) - pp*dt*((pos_e(1))/r_es**3 + pj*(pos_e(1)-pos_j(1))/r_ej**3)
            vel_e(2) = vel_e(2) - pp*dt*((pos_e(2))/r_es**3 + pj*(pos_e(2)-pos_j(2))/r_ej**3)
            vel_j(1) = vel_j(1) - pp*dt*((pos_j(1))/r_js**3 + pe*(pos_j(1)-pos_e(1))/r_ej**3)
            vel_j(2) = vel_j(2) - pp*dt*((pos_j(2))/r_js**3 + pe*(pos_j(2)-pos_e(2))/r_ej**3)
    !
    ...
    !            
            if(mod(i,100)==0) write(u,'(10f14.7)')  t,pos_e(1),pos_e(2),pos_j(1),pos_j(2)
        end do
